...
import com.msix.admin.review.service.AReviewService;
...
public class AReviewController {
	...
	private AReviewService reviewService;
	
	//리뷰게시판 리스트 (페이징 처리 목록 조회)
	@GetMapping("/reviewList")
	public String boardList(@ModelAttribute("data") AReviewVO rvo, Model model) {
		log.info("review boardList 조회");
		List<AReviewVO> boardList = reviewService.boardList(rvo);
	...
	public String boardDetail(@ModelAttribute AReviewVO rvo, Model model) {
		AReviewVO detail = reviewService.boardDetail(rvo);
	...
	public String boardDelete(@ModelAttribute AReviewVO rvo, RedirectAttributes ras) {
		int result = 0;
		String url = "";
